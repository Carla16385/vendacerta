...
export function OnboardingFlow() {
  ...
  const go = (n: Screen) => { setScreen(n); window.scrollTo(0, 0) }
  return (
    <main className="mx-auto min-h-dvh max-w-md bg-white">
      ...
      {screen === 6 && (
        <div className="flex min-h-dvh flex-col items-center justify-center p-6 text-center">
          <span className="flex h-20 w-20 items-center justify-center rounded-full bg-green-100"><PartyPopper className="h-10 w-10 text-green-600" /></span>
          <h2 className="mt-6 text-2xl font-black">Tudo pronto! 🎉</h2>
          <p className="mt-2 text-zinc-600">Seus vídeos foram agendados! Agora desbloqueie sua loja completa!</p>
          <div className="mt-6 w-full rounded-2xl bg-yellow-50 p-4 border border-yellow-200"><p className="text-sm font-bold">🔥 OFERTA ESPECIAL</p><p className="mt-1 text-2xl font-black">R$27 <span className="text-sm font-normal line-through">R$97</span></p><p className="text-xs text-zinc-600">Acesso vitalício + vídeos prontos</p></div>
          <button onClick={() => window.location.href = 'https://pay.kiwify.com.br/Vtt0mHD'} className="mt-6 w-full rounded-full bg-[#FF6B00] py-4 text-lg font-black text-white shadow-lg">LIBERAR MINHA LOJA AGORA 🚀</button>
